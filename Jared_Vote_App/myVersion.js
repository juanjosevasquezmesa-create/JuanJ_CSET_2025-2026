...
class System {
    ...
    loadUsers(filename = "users.json") {
        ...
        this.users = usersArray.map(u => {
            ...
            user.votes = u.votes || {};//it is empty in the senario in which the user
            return user;
        });
    ...
    checkAuth(user, inputPassword) {
        if (!inputPassword) return false;//this checks if the user enters anything if empty it will be null (0, whcih is false to JS) (so the opposite is true)
